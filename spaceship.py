...
class Sprite:
    ...
    def collide(self, other_object):
        if math.sqrt((self.pos[0]-other_object.get_position()[0])**2+(self.pos[1]-other_object.get_position()[1])**2) <= (self.get_radius()+ other_object.get_radius()):
            return True
        else:
            return False
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        rock_vel = [random.random() * .6 + score / 30 - .3, random.random() * .6 + score / 30 - .3]
        rock_avel = random.random() * .2 - .1
        if math.sqrt((rock_pos[0]-my_ship.get_position()[0])**2+(rock_pos[1]-my_ship.get_position()[1])**2) > (asteroid_info.get_radius()+ my_ship.get_radius()): 
            rock_group.add(Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info))

def process_sprite_group(rocks, canvas):
    copy_group = set(rocks)
    for rock in copy_group:
        rock.draw(canvas)
        if rock.update():
            rocks.remove(rock)    

def group_collide(other_object):
    global rock_group
...
def group_group_collide(first_group, second_group):
    global score
    copy_group = set(first_group)  
    for element in copy_group:
        if group_collide(element):
